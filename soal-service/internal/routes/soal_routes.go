...
func InitializeRoutes(r *gin.Engine, soalHandler *handlers.SoalHandler) {
	...
	internalSoal := r.Group("/internal/soal")
	internalSoal.Use(utils.RequireInternalServiceToken())
	{
		internalSoal.GET("/answer-key/:paket_soal", soalHandler.GetAnswerKeyByPaketAndSubtest)
	}
}
